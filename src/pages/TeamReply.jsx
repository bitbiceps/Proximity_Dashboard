...
const TeamMessage = () => {
  ...
  const { teamReply , id} = userData
  const [userTeamMessage , setUserTeamMessage] = useState(teamReply);
  ...
  useEffect(() => {
    const { teamReply} = userData
    setUserTeamMessage(teamReply)     
  } , [userData])
  ...
  return (
    <RootLayout2>
          <div className="flex justify-between items-center p-3">
            ...
            <button className="flex items-center justify-between gap-1 bg-app-blue-1 text-white hover:scale-110 p-2 rounded-md transition-all"
            onClick={handleRefreshMessage}
            >
                    <MdRefresh className={`${loading ? "animate-spin" : ""}`}  size={24}/> <div className="font-semibold text-sm md:text-sm">{loading ? "loading..":"Refresh"}</div> 
            </button>
          </div>
      {
        userTeamMessage && userTeamMessage?.length > 0 ? <div className="flex flex-col  min-h-[95vh] md:min-h-[70vh] 2xl:h-[90vh] p-4 rounded-lg shadow-md">

          <div className="w-full px-2 mt-8  grid grid-cols-1 md:grid-cols-3 gap-4">
            {

              userTeamMessage.map((item, index) => <div className="w-full  p-3 flex flex-col justify-between border-2 border-gray-300 rounded-md" key={index}>
        ...
        </div> : <div className="flex flex-col items-center justify-center  min-h-[95vh] md:min-h-[70vh] 2xl:h-[90vh] rounded-lg shadow-md">
